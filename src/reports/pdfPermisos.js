...
export const generarPDFPermisos = (datos, filtros) => {
  ...
  doc.text(`ID: REP-PER-${Date.now()}`, 260, 30, { align: 'right' });

  doc.setFont('helvetica', 'bold');
  doc.setFontSize(18);
  doc.setTextColor(0, 45, 107);
  doc.text('REPORTE DE PERMISOS', 15, 42);
  ...
  doc.autoTable({
    ...
    theme: 'plain',
    styles: { font: 'helvetica', fontSize: 9, cellPadding: 3.5, lineColor: [210, 215, 223], lineWidth: 0.3, valign: 'middle' },
    headStyles: { fillColor: [208, 227, 252], textColor: [0, 45, 107], fontStyle: 'bold', halign: 'center', lineWidth: 0.3 },
    ...
    didParseCell(data) {
      if (data.section === 'body' && data.column.dataKey === 'estado') {
        ...
        if (v === 'APROBADO') { data.cell.styles.textColor = [46, 125, 50]; data.cell.styles.fontStyle = 'bold'; }
        else if (v === 'PENDIENTE') { data.cell.styles.textColor = [239, 108, 0]; data.cell.styles.fontStyle = 'bold'; }
        else if (v === 'RECHAZADO') { data.cell.styles.textColor = [198, 40, 40]; data.cell.styles.fontStyle = 'bold'; }
      }
    },
    didDrawPage(data) {
  ...
  doc.save(`Reporte_Permisos_${filtros.fecha_inicio}_${filtros.fecha_fin}.pdf`);
};