  ...
  const cargarDatos = async () => {
    ...
    const [{ data: emps }, { data: asis }] = await Promise.all([
      supabase
        ...
        .order("nombre"),
      supabase.from("asistencia").select("*").eq("fecha", fechaFiltro),
    ]);
    setEmpleados(emps || []);
    setAsistencias(asis || []);
    setLoading(false);
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          {fechaFiltro !== hoy && (
            <button
              ...
              className="text-xs font-semibold text-[#004bb4] hover:underline whitespace-nowrap"
            >
              Hoy
            </button>
          )}
                  ...
                  empleadosFiltrados.map((emp) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-5 py-3 text-slate-500">{fechaFiltro}</td>
                        ...
                        <td className="px-5 py-3">
                          {asis ? (
                            <span
                              className={`${ESTADO_BADGE[asis.estado] ?? "bg-slate-50 text-slate-500"} px-2 py-0.5 rounded-md font-bold text-[10px] capitalize`}
                            >
                              {asis.estado}
                            </span>
                          ) : (
                            <span className="bg-slate-100 text-slate-400 px-2 py-0.5 rounded-md font-bold text-[10px]">
                              Sin registro
                            </span>
                          )}
                        </td>
          ...
          <form onSubmit={handleGuardarEdicion} className="space-y-4">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormEditar({ ...formEditar, hora_entrada: e.target.value })
                }
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormEditar({ ...formEditar, hora_salida: e.target.value })
                }
                className={INPUT_BASE}
              />
            </div>
