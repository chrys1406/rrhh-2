...
export default function Tardanzas() {
  ...
  const cargarDatos = async () => {
    ...
    const masTardanzas = Object.entries(conteo).sort((a, b) => b[1] - a[1])[0]?.[0] ?? '—';

    // Promedio minutos tarde
    const minutos = todas.map(t => calcularMinutosTarde(t.hora_entrada));
    const promedio = minutos.length > 0
              ...
              <tbody className="divide-y divide-slate-50">
                {tardanzasFiltradas.length === 0 ? (
                  ...
                  tardanzasFiltradas.map((t) => {
                    ...
                    return (
                      <tr key={t.id_asistencia} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="px-5 py-3 text-slate-500">{t.empleado?.area?.nombre_area}</td>
                        <td className="px-5 py-3 text-slate-500">{t.fecha}</td>
                        <td className="px-5 py-3 font-semibold text-slate-700">{t.hora_entrada}</td>
                        <td className="px-5 py-3 text-slate-400">08:30</td>
                        <td className="px-5 py-3">
                          <span className="font-black text-slate-800">+{minutos} min</span>
                        </td>
                        <td className="px-5 py-3">
                          <span className={`${getBadgeMinutos(minutos)} px-2 py-0.5 rounded-md font-bold text-[10px]`}>
                            {minutos <= 15 ? 'Leve' : minutos <= 30 ? 'Moderada' : 'Grave'}
                          </span>
                        </td>
                      </tr>
                    );
