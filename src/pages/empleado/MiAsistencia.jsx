  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="bg-white rounded-2xl shadow-sm px-6 py-5">
        <h3 className="text-sm font-bold text-slate-900 mb-4">Registro del Día</h3>
        <div className="flex flex-col sm:flex-row gap-4 items-center">

          {/* Estado actual */}
          <div className={`flex-1 rounded-xl px-5 py-4 flex items-center gap-3 ${
            ...
            <div>
              <p className="text-sm font-bold text-slate-800">
                {!asistenciaHoy ? 'Sin registro hoy' :
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                {!asistenciaHoy ? 'Marca tu entrada para comenzar' :
                 asistenciaHoy.hora_salida ? '¡Jornada completada!' :
          ...
          <div className="flex gap-3 shrink-0">
            <button
              ...
              className="flex items-center gap-2 bg-[#004bb4] hover:bg-[#003785] disabled:opacity-40 disabled:cursor-not-allowed text-white text-sm font-semibold px-5 py-3 rounded-xl transition-all shadow-lg shadow-blue-600/20"
            >
              <LogIn size={16} /> Marcar Entrada
            </button>
            <button
              onClick={marcarSalida}
              disabled={!asistenciaHoy || !!asistenciaHoy?.hora_salida || marcando}
              className="flex items-center gap-2 bg-red-500 hover:bg-red-600 disabled:opacity-40 disabled:cursor-not-allowed text-white text-sm font-semibold px-5 py-3 rounded-xl transition-all shadow-lg shadow-red-500/20"
            >
      ...
      <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100">
          <h3 className="text-sm font-bold text-slate-900">Historial del Mes</h3>
          <p className="text-[11px] text-slate-400 mt-0.5">{asistencias.length} registros este mes</p>
        </div>
          ...
          <table className="w-full text-left border-collapse text-xs">
            ...
            <tbody className="divide-y divide-slate-50">
              {asistencias.length === 0 ? (
                ...
                asistencias.map((a) => (
                  <tr key={a.id_asistencia} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-5 py-3 font-medium text-slate-700">{a.fecha}</td>
                    <td className="px-5 py-3 text-slate-500">{a.hora_entrada ?? '—'}</td>
                    <td className="px-5 py-3 text-slate-500">{a.hora_salida ?? '—'}</td>
                    <td className="px-5 py-3">
                      <span className={`${ESTADO_BADGE[a.estado] ?? 'bg-slate-50 text-slate-500'} px-2 py-0.5 rounded-md font-bold text-[10px] capitalize`}>
                        {a.estado}
                      </span>
                    </td>
                  </tr>
