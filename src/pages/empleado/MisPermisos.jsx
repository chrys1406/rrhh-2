...
export default function MisPermisos() {
  ...
  const handleEliminar = async () => {
    await supabase
      .from("permiso")
      .delete()
      .eq("id_permiso", modalEliminar.id_permiso);
                  ...
                  {permisos.map((p) => (
                    ...
                    >
                      ...
                      <td className="px-5 py-3">
                        <span
                          className={`${ESTADO_BADGE[p.estado] ?? "bg-slate-50 text-slate-500"} px-2 py-0.5 rounded-md font-bold text-[10px] capitalize`}
                        >
                          {p.estado}
                        </span>
                      </td>
                ...
                >
                  <div className="flex justify-between items-start gap-2">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-1">{p.motivo}</p>
                    </div>
                    <span
                      className={`${ESTADO_BADGE[p.estado] ?? "bg-slate-50 text-slate-500"} px-2 py-0.5 rounded-md font-bold text-[10px] capitalize shrink-0`}
                    >
                      {p.estado}
                    </span>
                  </div>
      ...
      {modalEliminar && (
        ...
        >
          <p className="text-sm text-slate-600 mb-6">
            ¿Estás seguro de eliminar esta solicitud de permiso? Esta acción no
            se puede deshacer.
          </p>
