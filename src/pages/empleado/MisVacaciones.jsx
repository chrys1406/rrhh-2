...
export default function MisVacaciones() {
  ...
  const handleEliminar = async () => {
    await supabase.from('vacacion').delete().eq('id_vacacion', modalEliminar.id_vacacion);
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="bg-white rounded-2xl px-6 py-4 shadow-sm flex justify-between items-center">
        <div>
          <p className="text-sm font-bold text-slate-800">Solicitar vacaciones</p>
      ...
      <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100">
          <h3 className="text-sm font-bold text-slate-900">Historial de Vacaciones</h3>
        </div>
          ...
          <table className="w-full text-left border-collapse text-xs">
            ...
            <tbody className="divide-y divide-slate-50">
              {vacaciones.length === 0 ? (
                ...
                vacaciones.map((v) => (
                  <tr key={v.id_vacacion} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-5 py-3 text-slate-700 font-medium">{v.fecha_inicio}</td>
                    <td className="px-5 py-3 text-slate-500">{v.fecha_fin}</td>
                    <td className="px-5 py-3">
                      <span className="font-bold text-slate-800">{v.dias} días</span>
                    </td>
                    <td className="px-5 py-3">
                      <span className={`${ESTADO_BADGE[v.estado] ?? 'bg-slate-50 text-slate-500'} px-2 py-0.5 rounded-md font-bold text-[10px] capitalize`}>
                        {v.estado}
                      </span>
                    </td>
      ...
      {modalAbierto && (
        <Modal
          titulo={editando ? 'Editar Solicitud' : 'Nueva Solicitud de Vacaciones'}
          onClose={cerrarModal}
        >
          <form onSubmit={handleGuardar} className="space-y-4">
            ...
            {dias > 0 && (
              <div className="bg-blue-50 border border-blue-100 rounded-xl px-4 py-3">
                <p className="text-xs text-[#004bb4] font-semibold">
                  📅 Total de días solicitados: <span className="text-lg font-black">{dias}</span>
                </p>
              </div>
            )}
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <button type="submit" disabled={guardando || dias === 0}
      ...
      {modalEliminar && (
        <Modal titulo="Eliminar Solicitud" onClose={() => setModalEliminar(null)}>
          <p className="text-sm text-slate-600 mb-6">
            ¿Estás seguro de eliminar esta solicitud de vacaciones? Esta acción no se puede deshacer.
          </p>
